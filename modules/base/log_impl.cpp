                   ...
                   int line, int level, int with_args, const char *fmt, ...)
{
    ...
    uint32_t sec = static_cast<uint32_t>(now_time_t);
    uint32_t usec = static_cast<uint32_t>(now_microseconds % 1000000);

    LogContent content = {
        .thread_id = sys::gettid(),
        .timestamp = {
            .sec  = static_cast<uint32_t>(sec),
            .usec = static_cast<uint32_t>(usec),
        },
    ...
    if (fmt != nullptr) {
        if (with_args) {
            uint32_t buff_size = std::min(2048lu, (unsigned long)_LogTextMaxLength) + 1;
